...
public class TypePresenter extends BasePresenter<TypeView> {
    ...
    public void getTagData() {
        ...
        RetrofitManage.getInstents().onCreate(Api.class)
                .getTypeTagData()
                .enqueue(new RequestCallBack<ResponseData<List<TypeTagVO>>>() {
                    @Override
                    public void onSuccessful(Call<ResponseData<List<TypeTagVO>>> call, Response<ResponseData<List<TypeTagVO>>> response) {
                        mTagDatas = response.body().getData();
                        setTabUI();
                        mTabSelect = 0;
                        mTagSelect = 0;
    ...
    public void getServerData(int cid) {
        ...
        RetrofitManage.getInstents().onCreate(Api.class)
                .getTypeDataById(mCurrentPage, cid)
                .enqueue(new RequestCallBack<ResponseData<ArticleListVO>>() {
                    ...
                    public void onSuccessful(Call<ResponseData<ArticleListVO>> call, Response<ResponseData<ArticleListVO>> response) {
                        if (response.body().getData().getDatas() != null) {
    ...
    public void getMoreData() {
        ...
        RetrofitManage.getInstents().onCreate(Api.class)
                .getTypeDataById(mCurrentPage, mId)
                .enqueue(new RequestCallBack<ResponseData<ArticleListVO>>() {
